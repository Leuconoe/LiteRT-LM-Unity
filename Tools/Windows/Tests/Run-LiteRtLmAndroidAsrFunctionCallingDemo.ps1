...
if (!(Test-Path $ApkPath)) {
    throw "ASR function-calling demo APK not found: $ApkPath"
}

$LogDirectory = Join-Path $ProjectRoot "Builds\Logs\AndroidDeviceRuns"
New-Item -ItemType Directory -Force -Path $LogDirectory | Out-Null
$RunId = Get-Date -Format "yyyyMMdd-HHmmss"
$rawLog = Join-Path $LogDirectory "$RunId-asr-function-calling.logcat.txt"
$summaryLog = Join-Path $LogDirectory "$RunId-asr-function-calling.summary.txt"
$statusLog = Join-Path $LogDirectory "$RunId-asr-function-calling.status.txt"
$deviceStatusPath = "/sdcard/Android/data/$PackageName/files/LiteRtLmAsrFunctionCallingDemo.status.txt"
...
Write-Host "[LiteRT-LM] Installing ASR function-calling demo APK: $ApkPath"
...
$patterns = "LiteRT-LM ASRFunctionCalling|ASR_RESULT|TRANSCRIPT|LLM_INITIALIZED|FUNCTION_CALL|SUCCESS|FAILURE|AndroidRuntime|FATAL|ERROR|OpenCL|WebGPU|GPU"
...
if ([string]::IsNullOrWhiteSpace($statusText)) {
    throw "ASR function-calling demo timed out without a status file. Log: $summaryLog"
}

if ($statusText -match "(?m)\]\s+SUCCESS:") {
    Write-Host "[LiteRT-LM] ASR function-calling demo passed. Summary: $summaryLog"
    exit 0
}

throw "ASR function-calling demo failed or timed out. Summary: $summaryLog"
