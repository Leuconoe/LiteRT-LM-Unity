...
2. **litertlm GPU is broken on this Windows build.** All three `--backend=gpu` attempts produced empty stdout. The log shows the WebGPU delegate initializing successfully on the RTX 4090 via Direct3D 12 ("Created a WebGPU environment", "Failed to create OpenCL context" is informational), then the process dies silently during weight upload (`# of threads to upload weights = 2` is the last line). Reproduced 3/3 (`results/litertlm-gpu*.err`). Numbers are from the May 8 exe; retest when the v0.14 Windows exe lands.
