...
namespace LiteRTLM.Unity
{
    public sealed class LiteRtLmAsrTestRunner : MonoBehaviour, ILiteRtLmModelHost
    {
        ...
        // Plain running transcript, kept apart from the metrics log so the text
        // itself stays readable and copyable while a session is running.
        private readonly List<string> _transcriptOnly = new List<string>();
        private Vector2 _transcriptScroll;
        private static readonly string[] LanguageOptions = { "ko", "en", "auto" };
        private static readonly string[] BackendOptions = { "CPU", "GPU" };
        private bool _languageExpanded;
        private bool _backendExpanded;
        private Vector2 _panelScroll;
        private Rect _asrOutputRect;
        ...
        /// <summary>Adds one recognised utterance to the running transcript view.</summary>
        private void AppendTranscript(string transcript)
        {
            LiteRtLmUi.AppendLog(_transcriptOnly, $"[{LiteRtLmUi.Stamp()}] {transcript.Trim()}", 200);
        ...
        private void OnGUI()
        {
            ...
            // Two-column frame shared with the other sample scenes: controls left,
            // output right, navigator strip kept clear.
            LiteRtLmUi.BeginScreen("ASR", out var controlRect, out var outputRect);
            _asrOutputRect = outputRect;

            LiteRtLmUi.BeginPanel(controlRect, "ASR test");
            _panelScroll = GUILayout.BeginScrollView(_panelScroll);
            LiteRtLmUi.Status(
                _status,
                _isBusy ? LiteRtLmUi.StatusKind.Busy
                : _status != null && _status.StartsWith("Error", StringComparison.OrdinalIgnoreCase)
                    ? LiteRtLmUi.StatusKind.Error
                    : LiteRtLmUi.StatusKind.Idle);
            if (_isBusy)
            {
            ...
            {
                // No longer a dead end on desktop: transcription falls back to the
                // advanced CLI (gemma-4 audio), a different engine from the
                // on-device whisper path.
                GUILayout.Label(
                    LiteRtLmDesktopAsr.IsSupported
                        ? "Android bridge unavailable — transcribing with the Windows CLI (gemma-4 audio)."
                        : "ASR needs an Android build or the Windows CLI.",
                    LiteRtLmUi.Mono);
            }
            ...
            // Dropdowns rather than free-text: these are fixed enumerations, and a
            // typo in a text field silently produced an unsupported backend.
            var controlWidth = LiteRtLmUi.ControlColumnWidth - 40f;
            var languageIndex = Mathf.Max(0, System.Array.IndexOf(LanguageOptions, language));
            languageIndex = LiteRtLmUi.Dropdown(
                "Language", languageIndex, LanguageOptions, ref _languageExpanded, controlWidth, !_isBusy);
            language = LanguageOptions[languageIndex];

            var backendIndex = Mathf.Max(0, System.Array.IndexOf(BackendOptions, backend));
            backendIndex = LiteRtLmUi.Dropdown(
                "Backend", backendIndex, BackendOptions, ref _backendExpanded, controlWidth, !_isBusy);
            backend = BackendOptions[backendIndex];
            ...
            else
            {
                ...
                if (GUILayout.Button("Transcribe", GUILayout.Height(36), GUILayout.ExpandWidth(true)))
                {
            ...
            LiteRtLmUi.BeginPanel(_asrOutputRect, "Output");
            GUILayout.BeginHorizontal();
            GUILayout.Label("Transcript", LiteRtLmUi.SectionHeader);
            GUILayout.FlexibleSpace();
            if (GUILayout.Button("Copy", GUILayout.Width(56f)) && _transcriptOnly.Count > 0)
            {
                GUIUtility.systemCopyBuffer = string.Join(Environment.NewLine, _transcriptOnly);
                _status = "Transcript copied to clipboard";
            }

            if (GUILayout.Button("Clear", GUILayout.Width(56f)))
            {
                _transcriptOnly.Clear();
            }

            GUILayout.EndHorizontal();

            // Selectable so the tester can copy what was actually recognised.
            _transcriptScroll = GUILayout.BeginScrollView(_transcriptScroll, GUILayout.Height(120));
            GUILayout.TextArea(
                _transcriptOnly.Count == 0 ? "(nothing transcribed yet)" : string.Join(Environment.NewLine, _transcriptOnly),
                LiteRtLmUi.Mono,
                GUILayout.ExpandHeight(true));
            GUILayout.EndScrollView();

            GUILayout.Space(6);
            GUILayout.Label("Metrics / raw log", LiteRtLmUi.SectionHeader);
            _logScroll = GUILayout.BeginScrollView(_logScroll, GUILayout.Height(300));
        ...
        private void DrawModelDropdown()
        {
            ...
            {
                GUILayout.Label("No ASR model options configured.", LiteRtLmUi.Mono);
                return;
            }
            ...
            {
                labels[i] = GetOptionLabel(modelOptions[i]);
            }

            selectedModelIndex = LiteRtLmUi.Dropdown(
                "ASR model",
                Mathf.Clamp(selectedModelIndex, 0, modelOptions.Length - 1),
                labels,
                ref _modelListExpanded,
                LiteRtLmUi.ControlColumnWidth - 40f,
                !_isBusy);

            // Say so rather than silently ignoring the choice: the Whisper
            // tflite tiers run on the Android bridge, while the desktop CLI is an
            // LLM runner and only does audio through gemma-4. Selecting
            // "Whisper Base ACFT-ko" here and reading "gemma-4 audio" in the
            // results was confusing with no explanation on screen.
            if (!_client.IsAvailable && LiteRtLmDesktopAsr.IsSupported)
            {
                GUILayout.Label(
                    "Applies to Android builds. This editor transcribes with the Windows CLI (gemma-4 audio).",
                    LiteRtLmUi.Mono);
            }
        }

        /// <summary>
        /// Clip picker. Uses the shared dropdown so long Korean filenames are
        /// ellipsized instead of stretching the control column; the full name is
        /// still available as the tooltip.
        /// </summary>
        private void DrawAudioDropdown()
        {
            var labels = new string[AudioOptions.Length];
            for (var i = 0; i < AudioOptions.Length; i++)
            {
                labels[i] = System.IO.Path.GetFileNameWithoutExtension(AudioOptions[i]);
            }

            selectedAudioIndex = LiteRtLmUi.Dropdown(
                "Audio",
                Mathf.Clamp(selectedAudioIndex, 0, AudioOptions.Length - 1),
                labels,
                ref _audioListExpanded,
                LiteRtLmUi.ControlColumnWidth - 40f,
                !_isBusy);
        }
        ...
        {
            selectedVadModeIndex = LiteRtLmUi.Dropdown(
                "VAD mode",
                Mathf.Clamp(selectedVadModeIndex, 0, VadModeLabels.Length - 1),
                VadModeLabels,
                ref _vadModeListExpanded,
                LiteRtLmUi.ControlColumnWidth - 40f,
                !_isBusy);
        }
        ...
        private void DrawMicSection(AsrModelOption selectedOption)
        {
            ...
            {
                GUILayout.Label($"Last capture: {Path.GetFileName(_lastMicWavPath)}", LiteRtLmUi.Mono);
            }
        ...
        private void HandleMicUtteranceCaptured(float[] pcm16k)
        {
            ...
            // The saved wav path is a debugging detail; the transcript is what the
            // tester is here for. Path stays available in _lastMicWavPath.
            _status = $"Captured {utteranceSeconds:0.00}s - transcribing...";

            var option = GetSelectedModelOption();
            if (!_client.IsAvailable)
            {
                if (LiteRtLmDesktopAsr.IsSupported && !_isBusy)
                {
                    StartCoroutine(DesktopTranscribeRoutine(wavPath, MicAudioLabel));
                }
                else
                {
                    _status = "Mic capture saved (no ASR path on this platform)";
                }
        ...
        private void StartContinuousSession(AsrModelOption option)
        {
            ...
            // Task thread only calls methods on the existing object. There is no
            // bridge on desktop, where the worker shells out to the CLI instead.
            if (_client.IsAvailable)
            {
        ...
        private void HandleContinuousUtteranceCaptured(float[] pcm16k)
        {
            ...
            var captureLine = $"[{DateTime.Now:HH:mm:ss}] #{index} captured {utteranceSeconds:0.00}s (queue={_utteranceQueue.Count}/{MaxQueuedUtterances})";
        ...
        private IEnumerator ContinuousWorkerRoutine(AsrModelOption option)
        {
            ...
            // Desktop has no Java bridge, so the session used to capture WAVs and
            // transcribe none of them. The Windows CLI handles each utterance
            // instead; the on-device model/tokenizer resolution below applies to
            // the Android path only.
            var useDesktopCli = !_client.IsAvailable && LiteRtLmDesktopAsr.IsSupported;
            var canTranscribe = option != null && (_client.IsAvailable || useDesktopCli);
            if (!canTranscribe)
            {
                Debug.LogWarning("LiteRtLmAsrTestRunner continuous: captures will be saved but not transcribed (no model selected or no ASR path on this platform)");
            }
            else if (useDesktopCli)
            {
                Debug.Log("LiteRtLmAsrTestRunner continuous: transcribing through the Windows CLI");
            }
            else
            {
            ...
            while (!_continuousStopRequested)
            {
                ...
                if (!canTranscribe)
                {
                    _transcriptLog.Add($"[{DateTime.Now:HH:mm:ss}] #{item.Index} saved, ASR unavailable on this platform");
                    _logScroll = new Vector2(0f, float.MaxValue);
                    continue;
                }
                ...
                if (useDesktopCli)
                {
                    LiteRtLmDesktopAsr.Result desktopResult = null;
                    yield return LiteRtLmDesktopAsr.Transcribe(
                        item.WavPath, language, backend, r => desktopResult = r);

                    var desktopSeconds = Time.realtimeSinceStartup - asrStartedAt;
                    var desktopLatency = Time.realtimeSinceStartup - item.EndpointedAt;
                    if (desktopResult == null || !desktopResult.Success)
                    {
                        var desktopError = desktopResult?.Error ?? "unknown failure";
                        var desktopErrorLine = $"[{DateTime.Now:HH:mm:ss}] #{item.Index} ASR ERROR: {desktopError}";
                        Debug.LogWarning($"LiteRtLmAsrTestRunner continuous: {desktopErrorLine}");
                        _transcriptLog.Add(desktopErrorLine);
                        _logScroll = new Vector2(0f, float.MaxValue);
                        continue;
                    }

                    _sessionTranscribedCount++;
                    _lastUtteranceLatencySeconds = desktopLatency;
                    _transcriptLog.Add(
                        $"[{DateTime.Now:HH:mm:ss}] #{item.Index} ({item.Seconds:0.00}s) transcript={desktopResult.Transcript}" +
                        $"{Environment.NewLine}  latency={desktopLatency:0.00}s (asr={desktopSeconds:0.00}s) " +
                        $"desktop CLI queue={_utteranceQueue.Count}/{MaxQueuedUtterances}");

                    if (!string.IsNullOrWhiteSpace(desktopResult.Transcript))
                    {
                        AppendTranscript(desktopResult.Transcript);
                    }
                ...
                if (!failed && !string.IsNullOrWhiteSpace(transcript))
                {
        ...
        /// <summary>
        /// Desktop transcription through the advanced CLI (see LiteRtLmDesktopAsr).
        /// A different engine from the on-device whisper path, so results are
        /// labelled as such in the log.
        /// </summary>
        private IEnumerator DesktopTranscribeRoutine(string audioPath, string audioLabel)
        {
            _isBusy = true;
            _requestStartedAt = Time.realtimeSinceStartup;
            _status = "Transcribing (desktop CLI)...";

            LiteRtLmDesktopAsr.Result result = null;
            yield return LiteRtLmDesktopAsr.Transcribe(audioPath, language, backend, r => result = r);

            if (result == null || !result.Success)
            {
                var error = result?.Error ?? "unknown failure";
                _status = $"Error: {error}";
                _transcriptLog.Add($"[{DateTime.Now:HH:mm:ss}] desktop ASR failed: {error}");
                Debug.LogWarning($"[LiteRT-LM ASR] desktop ASR failed: {error}");
            }
            else
            {
                AppendTranscript(result.Transcript);
                _transcriptLog.Add(
                    $"[{DateTime.Now:HH:mm:ss}] desktop CLI (gemma-4 audio)  audio={audioLabel}" +
                    Environment.NewLine +
                    $"elapsed={result.ElapsedSeconds:0.00}s" + Environment.NewLine +
                    $"transcript={result.Transcript}");
                Debug.Log($"[LiteRT-LM ASR] desktop transcript ({result.ElapsedSeconds:0.0}s): {result.Transcript}");
                _status = $"Transcription complete ({result.ElapsedSeconds:0.0}s, desktop CLI)";
            }
        ...
        {
            // No Android bridge in the editor: route through the desktop CLI so the
            // scene actually transcribes instead of only saving a wav.
            if (!_client.IsAvailable && LiteRtLmDesktopAsr.IsSupported)
            {
                string desktopAudio = overrideAudioPath;
                if (string.IsNullOrEmpty(desktopAudio))
                {
                    desktopAudio = LiteRtLmStreamingAssets.Resolve(AudioOptions[selectedAudioIndex]);
                }

                yield return DesktopTranscribeRoutine(desktopAudio, audioLabel ?? AudioOptions[selectedAudioIndex]);
                yield break;
            }
