...
namespace LiteRTLM.Unity
{
    public class LiteRtLmSampleController : MonoBehaviour, ILiteRtLmModelHost
    {
        ...
        private void OnGUI()
        {
            ...
            // Screen-relative, navigator strip left clear — matches the other scenes.
            LiteRtLmUi.BeginScreen("Quick start", out var controlRect, out var outputRect);
            _outputRect = outputRect;
            LiteRtLmUi.BeginPanel(controlRect, "Quick start");
            ...
            if (canUseWindowsCli)
            {
                var width = LiteRtLmUi.ControlColumnWidth - 30f;
                ...
                // Dropdown rather than a free text field, as in every other scene:
                // a mistyped backend only failed later, inside the native call.
                var backendIndex = Array.IndexOf(BackendOptions, windowsBackend);
                backendIndex = LiteRtLmUi.Dropdown(
                    "Windows Backend",
                    backendIndex < 0 ? 0 : backendIndex,
                    BackendOptions,
                    ref _windowsBackendExpanded,
                    width,
                    !_isBusy);
                windowsBackend = BackendOptions[backendIndex];
            ...
            LiteRtLmUi.BeginPanel(_outputRect, "Response");
            ...
            Action<Exception> onError)
        {
            ...
            // On desktop, resolve by file name when the serialized path is stale —
            // scenes predating the StreamingAssets reorganisation still point at the
            // old flat layout.
            ...
            {
                var relocated = LiteRtLmStreamingAssets.Resolve(configuredPath, out var how);
                if (relocated != null)
                {
                    Debug.Log($"LiteRtLmSampleController: model '{configuredPath}' {how}.");
                    sourcePath = relocated.Replace("\\", "/");
                }
                else
                {
                    onError(new FileNotFoundException(
                        $"Model '{configuredPath}' was not found under StreamingAssets. " +
                        $"Available folders: {LiteRtLmStreamingAssets.DescribeAvailable()}. " +
