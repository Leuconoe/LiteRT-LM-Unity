...
namespace LiteRTLM.Unity
{
    ...
    public sealed class LiteRtLmTranslateTestRunner : MonoBehaviour, ILiteRtLmModelHost
    {
        ...
        private void OnGUI()
        {
            ...
            // Same two-column frame as the other sample scenes.
            LiteRtLmUi.BeginScreen("Translate", out var controlRect, out var outputRect);
            _outputRect = outputRect;
            LiteRtLmUi.BeginPanel(controlRect, "Translate");
            ...
            if (!_client.IsAvailable)
            {
                GUILayout.Label(
                    CanUseDesktopFallback
                        ? "Android bridge unavailable - using the Windows advanced CLI (gemma-4 audio) instead."
                        : "Translation requires an Android device build or the Windows CLI.",
                    LiteRtLmUi.Mono);
            }
            ...
            {
                DrawAudioDropdown();
                GUI.enabled = (_client.IsAvailable || CanUseDesktopFallback) && !_isBusy;
            ...
            LiteRtLmUi.BeginPanel(_outputRect, "Results");
            GUILayout.Label("Source transcript / translation / timings", LiteRtLmUi.SectionHeader);
        ...
        /// <summary>Clip picker. File names only — the folder prefix is noise.</summary>
        private void DrawAudioDropdown()
        {
            var labels = new string[AudioOptions.Length];
            for (var i = 0; i < AudioOptions.Length; i++)
            {
                labels[i] = Path.GetFileNameWithoutExtension(AudioOptions[i]);
            }
        ...
        /// <summary>
        /// True when the Windows advanced CLI can stand in for the Android bridge.
        /// It accepts audio as an `[audio:...]` prompt tag, so a multimodal model
        /// can transcribe and translate in one turn while running in the editor.
        /// </summary>
        private static bool CanUseDesktopFallback =>
            Application.platform == RuntimePlatform.WindowsEditor ||
            Application.platform == RuntimePlatform.WindowsPlayer;

        /// <summary>
        /// Desktop translation: transcribe through the shared desktop ASR helper
        /// (which converts non-wav input, since the CLI's [audio:] tag only decodes
        /// wav), then translate the transcript with the same CLI.
        /// </summary>
        private IEnumerator TranslateViaDesktopCli(string audioAbsolutePath, string targetLanguage)
        {
            var target = string.IsNullOrEmpty(targetLanguage) ? "English" : targetLanguage;

            _status = "Transcribing (desktop CLI)...";
            LiteRtLmDesktopAsr.Result asr = null;
            yield return LiteRtLmDesktopAsr.Transcribe(audioAbsolutePath, asrLanguage, asrBackend, r => asr = r);

            if (asr == null || !asr.Success)
            {
                var error = asr?.Error ?? "unknown failure";
                _status = $"Error: {error}";
                _resultLog.Add($"[{DateTime.Now:HH:mm:ss}] desktop ASR failed: {error}");
                Debug.LogWarning($"{LogPrefix} desktop ASR failed: {error}");
                _logScroll = new Vector2(0f, float.MaxValue);
                _isBusy = false;
                yield break;
            }

            _status = $"Translating into {target}...";
            ...
            {
                _status = modelPath == null ? "Translation model not found" : $"Windows CLI not found: {executable}";
                _isBusy = false;
                yield break;
            }
            ...
            var prompt =
                ...
                Environment.NewLine + Environment.NewLine + asr.Transcript;
            var cli = new LiteRtLmWindowsCliClient();
            var task = System.Threading.Tasks.Task.Run(() => cli.SendMessage(executable, modelPath, prompt, "CPU"));
            ...
            if (task.IsFaulted)
            {
                ...
                _status = $"Windows CLI failed: {error}";
                Debug.LogWarning($"{LogPrefix} translation failed: {error}");
                _isBusy = false;
                yield break;
            }

            var translation = task.Result?.Trim() ?? string.Empty;
            _resultLog.Add(
                $"[{DateTime.Now:HH:mm:ss}] desktop CLI  audio={Path.GetFileName(audioAbsolutePath)}  target={target}" +
                Environment.NewLine +
                $"transcript={asr.Transcript}" + Environment.NewLine +
                $"translation={(string.IsNullOrEmpty(translation) ? "(empty)" : translation)}" + Environment.NewLine +
                $"asr={asr.ElapsedSeconds:0.0}s");
            Debug.Log($"{LogPrefix} desktop translate: \"{asr.Transcript}\" -> \"{translation}\"");
            _logScroll = new Vector2(0f, float.MaxValue);
            _status = string.IsNullOrEmpty(translation) ? "Empty translation" : "Translation complete (desktop CLI)";
            _isBusy = false;
        }
        ...
        private void HandleMicUtteranceCaptured(float[] pcm16k)
        {
            ...
            _status = $"Captured {seconds:0.00}s - translating...";
            if (!_client.IsAvailable && !CanUseDesktopFallback)
            {
                _status = "Mic capture saved (translation needs the Android bridge or the Windows CLI)";
                return;
            }
        ...
        private IEnumerator TranslateRoutine(string audioSourcePath, string audioLabel)
        {
            ...
            // Desktop has no Android bridge and therefore no whisper tflite path;
            // route through the advanced CLI so the scene is testable in the editor.
            if (!_client.IsAvailable)
            {
                if (!CanUseDesktopFallback)
                {
                    _status = "Translation requires the Android bridge or the Windows CLI.";
                    _isBusy = false;
                    yield break;
                }

                var desktopAudio = Path.IsPathRooted(audioSourcePath)
                    ? audioSourcePath
                    : LiteRtLmStreamingAssets.Resolve(audioSourcePath);

                if (string.IsNullOrEmpty(desktopAudio) || !File.Exists(desktopAudio))
                {
                    _status = $"Audio not found: {audioLabel}";
                    _isBusy = false;
                    yield break;
                }

                var target = IsWhisperDirectEngine()
                    ? "English"
                    : TargetLanguages[Mathf.Clamp(selectedTargetLanguageIndex, 0, TargetLanguages.Length - 1)];

                yield return TranslateViaDesktopCli(desktopAudio, target);
                yield break;
            }
