...
namespace LiteRTLM.Unity
{
    public sealed class LiteRtLmConversationTestRunner : MonoBehaviour
    {
...

        {
            ...
            _overlay = LiteRtLmRunLogOverlay.Attach("Conversation regression test");
        ...
        private static string ResolveModelPath(string configuredPath)
        {
            ...
            // Falls back to a search by file name: scenes serialize this path, so a
            // scene authored before the StreamingAssets reorganisation still carries
            // the old flat name even though the script default was updated.
            var resolved = LiteRtLmStreamingAssets.Resolve(configuredPath, out var how);
            if (resolved != null)
            {
                if (how != null && how != "exact")
                {
                    Debug.Log($"LiteRtLmConversationTestRunner: model '{configuredPath}' {how}.");
                }
        ...
        {
            LiteRtLmRunLogOverlay.Current?.LogRaw($"[{phase}] {message}");
